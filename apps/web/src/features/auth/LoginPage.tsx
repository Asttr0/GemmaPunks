...
type DemoRole = "merchant" | "supplier";
...
export function LoginPage({
  ...
  const [organizationType, setOrganizationType] =
    useState<OrganizationType>("MERCHANT");
  const [showPassword, setShowPassword] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [demoSubmitting, setDemoSubmitting] = useState<DemoRole | null>(null);
  const [error, setError] = useState<string | null>(null);

  const busy = authLoading || submitting || demoSubmitting !== null;
        ...
        <section className="hidden min-h-dvh flex-col justify-between px-10 py-10 lg:flex xl:px-16 xl:py-12">
          ...
          >
            <div className="mb-6 inline-flex items-center gap-2 rounded-badge border border-violet-200 bg-ai-subtle px-3 py-1.5 text-sm font-semibold text-ai">
              ...
              Darija-first business intelligence
            </div>
            <h2 className="max-w-xl text-4xl font-bold leading-tight tracking-tight text-brand-950 xl:text-5xl">
              Turn everyday business evidence into purchasing power.
            </h2>
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-foreground-muted">
              Understand sales, protect your stock, compare supplier offers, and
              join nearby demand—all while you stay in control.
            </p>
            ...
            <ol className="mt-10 grid max-w-2xl gap-4 xl:grid-cols-3">
              <li className="rounded-card border border-border bg-surface/90 p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm leading-relaxed text-foreground-muted">
                  Voice, receipt, image, or a manual entry.
                </p>
              </li>
              <li className="rounded-card border border-border bg-surface/90 p-5 shadow-sm">
              ...
              <li className="rounded-card border border-border bg-surface/90 p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm leading-relaxed text-foreground-muted">
                  Reach wholesale quantities and save.
                </p>
              </li>
            </ol>
          </motion.div>
          ...
          <div className="w-full max-w-lg">
            <div className="mb-7 text-center lg:hidden">
              ...
              <p className="mt-2 text-sm text-foreground-muted">
                Business clarity. Better buying power.
              </p>
            </div>
              ...
              <CardContent className="p-6 sm:p-8">
                <div>
                  ...
                  <p className="mt-3 text-base leading-relaxed text-foreground-muted">
                    {isSignUp
                      ? "Choose how you use MIZAN and create your workspace."
                      : "Continue managing your business and purchasing needs."}
                  </p>
                </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="Berrechid Central Grocery"
                            className={inputClassName}
                          />
                        </div>
                        ...
                        <fieldset>
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            <label
                              className={cn(
                                "flex min-h-20 cursor-pointer items-center gap-3 rounded-control border p-3 outline-none transition-[background-color,border-color,box-shadow] duration-standard focus-within:ring-2 focus-within:ring-focus focus-within:ring-offset-2",
                            ...
                            >
                              <input
                                ...
                                value="MERCHANT"
                                checked={organizationType === "MERCHANT"}
                                disabled={busy}
                                onChange={() => setOrganizationType("MERCHANT")}
                                className="sr-only"
                              />
                            ...
                            <label
                              className={cn(
                                ...
                                organizationType === "SUPPLIER"
                                  ? "border-brand-500 bg-info-subtle shadow-sm"
                                  : "border-border bg-surface hover:border-border-strong hover:bg-surface-subtle",
                              )}
                            >
                              <input
                                ...
                                name="organization-type"
                                value="SUPPLIER"
                                checked={organizationType === "SUPPLIER"}
                                disabled={busy}
                                onChange={() => setOrganizationType("SUPPLIER")}
                                className="sr-only"
                              />
                              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-surface text-info shadow-sm">
                                <Building2
                                  aria-hidden="true"
                                  className="h-5 w-5"
                                />
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Button
                    variant="outline"
                    loading={demoSubmitting === "merchant"}
                    disabled={busy}
                    onClick={() => void handleDemoSignIn("merchant")}
                    className="w-full"
                  >
                    <Store
                      aria-hidden="true"
                  ...
                  <Button
                    variant="outline"
                    loading={demoSubmitting === "supplier"}
                    disabled={busy}
                    onClick={() => void handleDemoSignIn("supplier")}
                    className="w-full"
                  >
                    <Building2
                      aria-hidden="true"
                ...
                <div className="mt-6 flex items-start gap-3 rounded-control bg-surface-subtle p-4">
                  <CheckCircle2
                    aria-hidden="true"
                    className="mt-0.5 h-5 w-5 shrink-0 text-success"
                  />
                  <p className="text-sm leading-relaxed text-foreground-muted">
                    Demo accounts use synthetic hackathon data. No real merchant
                    records or payments are involved.
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="sr-only" aria-live="polite">
        {submitting
          ? isSignUp
            ...
            : "Signing you in"
          : demoSubmitting
            ? `Signing in to the ${demoSubmitting} demo`
            : ""}
      </div>
    </main>
  );
