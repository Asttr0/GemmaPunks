...
export function AgentTimeline({ run }: { run: AgentRunRecord }) {
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardHeader>
        <CardTitle>Visible tool-call timeline</CardTitle>
      </CardHeader>
      <CardContent>
                ...
                <div className="min-w-0 rounded-control border border-border bg-surface-subtle p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <StatusBadge
                      ...
                      tone={
                        successful ? "confirmed" : failed ? "failed" : "running"
                      }
                    />
                  </div>
        ...
        <p className="mt-6 border-t border-border pt-4 text-xs text-foreground-muted">
          Started {formatDateTime(run.created_at)}. AI created a draft; it did
          not confirm a financial record or order.
        </p>
      </CardContent>
    </Card>
  );
